...
export async function POST(request: NextRequest) {
  ...
  const res = await fetch(`${base}/api/automation/reports/send`, {
    ...
    body: JSON.stringify({ type: 'daily', recipients }),
  })
...
export async function GET() {
  return NextResponse.json({ webhook: '/api/automation/reports/daily', method: 'POST', description: 'n8n trigger para relatório diário' })
}
