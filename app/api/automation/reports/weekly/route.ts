...
export async function POST(_request: NextRequest) {
  ...
  const res = await fetch(`${base}/api/automation/reports/send`, {
    ...
    body: JSON.stringify({ type: 'weekly', recipients }),
  })
...
export async function GET() {
  return NextResponse.json({ webhook: '/api/automation/reports/weekly', method: 'POST', description: 'n8n trigger para relatório semanal' })
}
